...
OPENSHIFT_URL="https://192.168.99.100:8443"
...
oc create -f ../templates/java-redis-template.yaml -n $PROJECT_CICD
...
oc create -f ../templates/java-redis-template.yaml -n $PROJECT_DEV
oc create -f ../templates/java-redis-template.yaml -n $PROJECT_STAGE
