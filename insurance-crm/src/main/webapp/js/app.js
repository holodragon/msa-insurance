...
/* Uses JAX-RS GET to retrieve current products list */
function buildProductRows(products) {
    return _.template( $( "#product-tmpl" ).html(), {"products": products});
}
...
function updateProductTable() {
    ...
    $.ajax({
        ...
        cache: false,
        success: function(data) {
            $( "#products" ).empty().append(buildProductRows(data));
            $( "#product-table" ).table( "refresh" );
        },
        error: function(error) {
            //console.log("error updating table -" + error.status);
        },
